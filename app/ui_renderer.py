...
def _put(frame, text, x, y, scale=1.0, color=COLOR_WHITE, thickness=2):
    """Text with black stroke underneath for legibility on any background."""
...
def draw_alert(frame, text):
    h, w, _ = frame.shape
    box_y1, box_y2 = h - 100, h - 20

    good= text.lower().startswith("good")
    tint_color = (0, 50, 0) if good else (0, 0, 50)
    text_color = COLOR_GREEN if good else COLOR_RED

    overlay = frame.copy()
    cv2.rectangle(overlay, (20, box_y1), (w - 20, box_y2), tint_color, -1)
    cv2.addWeighted(overlay, 0.65, frame, 0.35, 0, frame)

    # Scale font to message length so it always fits
    scale= 1.8 if len(text) <= 12 else 1.3 if len(text) <= 18 else 1.0
    thickness = 3 if len(text) <= 12 else 2

    text_w = cv2.getTextSize(text, FONT, scale, thickness)[0][0]
    tx = max(24, (w - text_w) // 2)
    ty = box_y1 + int((box_y2 - box_y1) * 0.68)

    _put(frame, text, tx, ty, scale=scale, color=text_color, thickness=thickness)


def draw_rep_counter(frame, reps):
    _semi_rect(frame, 20, 20, 190, 80)
    _put(frame, f"Reps: {reps}", 36, 66, scale=1.5, color=COLOR_CYAN, thickness=3)
...
def draw_similarity_score(frame, score):
    h, w, _ = frame.shape
    color = COLOR_GREEN if score >= 0.82 else COLOR_ORANGE if score >= 0.70 else COLOR_RED
...
def draw_start_overlay(frame, seconds):
    ...
    _put(frame, "Get Ready",
         w // 2 - 120, h // 2 - 40, scale=1.5, color=COLOR_WHITE, thickness=3)
    _put(frame, str(seconds),
         w // 2 - 28,  h // 2 + 70, scale=3.0, color=COLOR_CYAN,  thickness=5)
...
def draw_exercise_intro(frame):
    ...
    lines = [
        ...
        ("", 0.8,  COLOR_WHITE),
        ("Feet shoulder-width apart", 0.75, COLOR_WHITE),
        ("Chest up, back straight", 0.75, COLOR_WHITE),
        ("Lower hips to knee level", 0.75, COLOR_WHITE),
